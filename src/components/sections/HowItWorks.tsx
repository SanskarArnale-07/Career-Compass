...
export function HowItWorks() {
  ...
  return (
    <section id="how-it-works" className="py-24 sm:py-32 w-full bg-muted/5 border-y border-border/30">
      <div className="container mx-auto px-4 max-w-5xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 relative">
          ...
          <div className="hidden md:block absolute top-10 left-[15%] right-[15%] h-px bg-border/80"></div>
          
          {steps.map((step, index) => (
            <div key={index} className="flex flex-col items-center text-center relative z-10">
              <div className="w-20 h-20 rounded-2xl bg-background border border-border flex items-center justify-center mb-6 shadow-sm">
                <step.icon className="h-8 w-8 text-primary" strokeWidth={1.5} />
              </div>
              <div className="mb-2">
                <span className="text-xs font-mono font-medium text-primary bg-primary/10 px-2 py-1 rounded-full">
                  Step {step.id}
                </span>
              </div>
